...
receiver = AudioReceiver(local_ip_adress,6424)
...
class ZoomApp(MDApp):
    ...
    def start_camera_streaming(self):
        self.camera_client = CameraClient("127.0.0.1",7777)
        self.t3 = threading.Thread(target=self.camera_client.start_stream)
        self.t3.start()
    
    def start_screen_sharing(self):
        self.screen_client = ScreenShareClient("127.0.0.1",7777)
        self.t4 = threading.Thread(target=self.screen_client.start_stream)
        self.t4.start()
        
    def start_Audio_streaming(self):
        self.audio_client = AudioSender("127.0.0.1",5555)
